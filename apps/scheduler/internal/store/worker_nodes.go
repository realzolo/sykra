...
func (s *Store) UpsertWorkerNode(ctx context.Context, node WorkerNode) error {
	...
	_, err = s.pool.Exec(
		...
		`insert into worker_nodes
		 ...
		 on conflict (id) do update set
		   ...
		   connected_at = coalesce(excluded.connected_at, worker_nodes.connected_at),
...
func (s *Store) MarkWorkerNodeOffline(ctx context.Context, workerID string, lastError *string) error {
	_, err := s.pool.Exec(
		ctx,
		`update worker_nodes
		 set status='offline',
