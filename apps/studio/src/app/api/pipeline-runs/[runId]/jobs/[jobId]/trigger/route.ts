...
import { triggerPipelineRunJobForOrg } from '@/features/pipeline-runs/application/managePipelineRunOperationsForOrg';
...
export const POST = withAuthedRoute<{ runId: string; jobId: string }>(
  ...
  async ({ request, params, user, orgId }) => {
    ...
    const result = await triggerPipelineRunJobForOrg({
      runId,
      jobId,
      body,
      orgId: orgId!,
