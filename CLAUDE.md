...
- **Pipeline static-analysis contract**: quality-gate static analysis should emit a SARIF artifact when the analyzer supports it, and Conductor writes a per-run changed-file manifest into the job workspace for scoped analyzers. Conductor ingests SARIF uploads and normalized `sykra.static-analysis.v1` JSON into structured quality-gate run events with tool metadata, severity counts, blocking counts, and sampled findings; Studio inference should prefer SARIF-producing commands and artifact paths for supported stacks and scope the analyzer to the changed-file manifest instead of relying on plain shell exit codes alone.
