...
- **Pipeline concurrency execution contract**: Conductor is the single execution authority for pipeline run concurrency across all trigger sources (`manual`, `webhook`, `schedule`). `queue` must enqueue and serialize execution by pipeline (oldest queued run first while no sibling run is `running`/`waiting_manual`), and `cancel_previous` must cancel existing `queued`/`running`/`waiting_manual` runs before creating the next run.
