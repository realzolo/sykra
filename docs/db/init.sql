...
create table org_runtime_settings (
...
create table codebase_thread_anchors (
...
create index idx_codebase_anchors_project on codebase_thread_anchors(project_id);
create index idx_codebase_anchors_file on codebase_thread_anchors(project_id, anchor_commit_sha, anchor_path);
...
create table codebase_thread_projections (
  thread_id uuid not null references codebase_comment_threads(id) on delete cascade,
  org_id uuid not null references organizations(id) on delete cascade,
  project_id uuid not null references code_projects(id) on delete cascade,
  repo text not null,
...
create index idx_codebase_projection_jobs_project
  on codebase_thread_projection_jobs(project_id, status);

-- ============================================================
-- Commit review markers
-- ============================================================
create table commit_review_items (
  id uuid primary key default gen_random_uuid(),
  org_id uuid not null references organizations(id) on delete cascade,
  project_id uuid not null references code_projects(id) on delete cascade,
  commit_sha text not null check (commit_sha ~* '^[0-9a-f]{7,40}$'),
...
create unique index uniq_commit_review_items
  on commit_review_items(project_id, commit_sha, path, line, reviewer_id);
create index idx_commit_review_items_commit
  on commit_review_items(project_id, commit_sha);
create index idx_commit_review_items_reviewer
...
create table artifact_version_usages (
  id uuid primary key default gen_random_uuid(),
  org_id uuid not null references organizations(id) on delete cascade,
  project_id uuid not null references code_projects(id) on delete cascade,
  repository_id uuid not null references artifact_repositories(id) on delete cascade,
...
create index artifact_repositories_project_idx on artifact_repositories (project_id, created_at desc);
create index artifact_versions_repository_idx on artifact_versions (repository_id, created_at desc);
create index artifact_versions_project_idx on artifact_versions (project_id, created_at desc);
create index artifact_files_version_idx on artifact_files (version_id, logical_path);
create index artifact_channels_repository_idx on artifact_channels (repository_id, updated_at desc);
create index artifact_version_usages_version_idx on artifact_version_usages (version_id, created_at desc);
create index artifact_version_usages_run_idx on artifact_version_usages (pipeline_run_id, created_at desc);
